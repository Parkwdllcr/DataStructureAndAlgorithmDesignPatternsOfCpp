...
#ifdef _UTILSBOX_OS_WIN_ // for windows impletement
#include <windows.h>
#include <direct.h>
#elif defined(_UTILSBOX_OS_LINUX_)
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include <time.h>
    ...
    *puiStringLen = 0;
    ...
    for ( uint32_t uiLoop=*puiDataLen;uiLoop>0;uiLoop-- )
    {
        if ( *pSEnd==chSeparator )
        {
            ...
            //skin space
            UTILSBOX_StrTrimSpace(*ppcTokenString, *puiStringLen);
            UTILSBOX_StrTrimLeftSpace(*ppcSipMsgBuf, *puiDataLen);
            return UTILSBOX_OK;
        }
        else
        {
...
int32_t CUtils::ExecuteSystemCommand(const char* strSystemCommand)
{
    ...
    while( 0<uiDataLen )
    {
        ...
        // get section
        if( UTILSBOX_OK!=CUtils::GetTokenString(&pcDataBuffer, &uiDataLen,' ', &pcTokenString, &uiStringLen) )
        {
            // last parameter
